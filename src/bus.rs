    ...
    Buffer: AsRef<[u8]> + AsMut<[u8]>,
{
    ...
    {
        self.write_packet(packet_id, instruction_id, parameter_count, encode_parameters).await?;
    ...
    async fn read_response_timeout(&mut self, timeout: Duration) -> Result<Response<&[u8]>, ReadError<SerialPort::Error>> {
    ...
    async fn read_packet_deadline(&mut self, deadline: SerialPort::Instant) -> Result<&[u8], ReadError<SerialPort::Error>> {
