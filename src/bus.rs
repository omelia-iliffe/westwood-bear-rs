...
pub struct Bus<SerialPort, Buffer = DefaultBuffer>
where
...
// The `open`/`open_with_buffers` convenience constructors are the same in both bisync
// trees (`<Port>::open` is a synchronous constructor for both the `serial2` and
// `serial2-tokio` ports), so they are generated by a macro and instantiated once per
// tree against that tree's default port type.
// Only invoked for whichever of the sync/async variants has its serial port backend
// enabled, so it is legitimately unused in the other variant's compilation.
#[allow(unused_macros)]
macro_rules! make_serial2_bus_impls {
    ($DefaultSerialPort:ty) => {
        ...
        impl<Buffer> Bus<$DefaultSerialPort, Buffer>
        where
            Buffer: AsRef<[u8]> + AsMut<[u8]>,
        {
            /// Open a serial port with the given baud rate.
